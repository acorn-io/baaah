...
type Options struct {
	...
	HealthProbeBindPort int // turn on healthcheck by specifying port
}
...
func NewRouter(handlerName string, scheme *runtime.Scheme, opts *Options) (*router.Router, error) {
	...
	return router.New(router.NewHandlerSet(handlerName, scheme, opts.Backend), opts.ElectionConfig, opts.HealthProbeBindPort), nil
}
