...
type RouteBuilder struct {
	...
	withoutPruneTypes []kclient.Object
	name              string
	namespace         string
	routeName         string
...
// WithoutPruneTypes specifies a list of types for which pruning will be disabled.
...
func (r RouteBuilder) WithoutPruneTypes(objTypes ...kclient.Object) RouteBuilder {
	r.withoutPruneTypes = append(r.withoutPruneTypes, objTypes...)
	return r
}
...
func (r RouteBuilder) Handler(h Handler) {
	...
	if len(r.withoutPruneTypes) > 0 {
		result = WithoutPruneTypesFilter{
			Next:              result,
			WithoutPruneTypes: r.withoutPruneTypes,
...
type WithoutPruneTypesFilter struct {
	Next              Handler
	WithoutPruneTypes []kclient.Object
	Backend           backend.Backend
}

func (w WithoutPruneTypesFilter) Handle(req Request, resp Response) error {
	gvks := make([]schema.GroupVersionKind, len(w.WithoutPruneTypes))
	for i, o := range w.WithoutPruneTypes {
		gvk, err := w.Backend.GVKForObject(o, w.Backend.Scheme())
		if err != nil {
			return err
		}
	...
	resp.WithoutPruneGVKs(gvks...)

	return w.Next.Handle(req, resp)
}
