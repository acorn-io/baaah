...
type RouteBuilder struct {
	...
	disabledPruneTypes []kclient.Object
	name               string
	namespace          string
	routeName          string
...
// DisablePruningForTypes specifies a list of types for which pruning will be disabled.
// This can be overridden on individual objects with the annotation apply.acorn.io/prune="true".
// This is useful for types that are generated by Kubernetes in most circumstances, such as Endpoints.
func (r RouteBuilder) DisablePruningForTypes(objTypes ...kclient.Object) RouteBuilder {
	r.disabledPruneTypes = append(r.disabledPruneTypes, objTypes...)
	return r
}
...
func (r RouteBuilder) Handler(h Handler) {
	...
	if len(r.disabledPruneTypes) > 0 {
		result = DisablePruningForTypesModifier{
			Next:                   result,
			DisablePruningForTypes: r.disabledPruneTypes,
...
type DisablePruningForTypesModifier struct {
	Next                   Handler
	DisablePruningForTypes []kclient.Object
	Backend                backend.Backend
}

func (d DisablePruningForTypesModifier) Handle(req Request, resp Response) error {
	gvks := make([]schema.GroupVersionKind, len(d.DisablePruningForTypes))
	for i, o := range d.DisablePruningForTypes {
		gvk, err := d.Backend.GVKForObject(o, d.Backend.Scheme())
		if err != nil {
			return err
		}
	...
	resp.DisablePruningForGVKs(gvks...)

	return d.Next.Handle(req, resp)
}
